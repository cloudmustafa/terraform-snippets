...
// InputParameters : Service name D passed in that
// will update the desired count and will stop
// all tasks
...
func updateService(sess *session.Session, serviceName string) error {
	...
	// Value is only to be 1
	const desiredCount = 1

	updateECS := &ecs.UpdateServiceInput{
		Cluster:            aws.String(varClusterName),
		DesiredCount:       aws.Int64(desiredCount), // This need to be greater than 0 to start the service and its tasks
