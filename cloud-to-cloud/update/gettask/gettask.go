...
// GetTask : Updates the desired count based on the service name.
// The service name is the "id" from the DynamoDB table and is the
// the CloudFormation StackName
func GetTask(sess *session.Session, subscription APISubscription) ([]*string, error) {

	svc := ecs.New(sess)

	var varClusterName = os.Getenv("CLUSTER_NAME")

	vTask := &ecs.ListTasksInput{
		Cluster:     aws.String(varClusterName),
		ServiceName: aws.String(subscription.ID),
	}
	results, err := svc.ListTasks(vTask)
	if err != nil {
